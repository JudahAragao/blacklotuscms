...
- **JWT via NextAuth v4:** Strategy JWT with PrismaAdapter
- **CredentialsProvider:** Autenticação por email + bcrypt password hash
- **Session Data:** Token contains user.id e user.role (with capabilities)
- **Custom Pages:** Login em /auth/login

## 2. Authorization (RBAC)

- **Capability Syshas:** Cada role has um JSON de capabilities (ex: `{ "post": { "create": true } }`)
- **Administrador:** Bypass automatic - all as capabilities retornam true
...
- **Rate Limiting:** Cache in-memory with janela de 1 minuto, limite configurável por chave
- **Proxy Validtion:** API keys são validadas no proxy antes de chegar às rotas

## 4. Input Validtion

- **Zod Schemas:** Todos os inputs de API validata via Zod (src/schemas/)
- **Path Sanitization:** `sanitizePath()` remove `..`, `/`, `\` para prevenir path traversal
- **HTML Sanitization:** DOMPurify with allowlist de tags para conteúdo de users
- **Slug Validtion:** Regex `^[a-z0-9-]+$` para slugs
...
- **Docker:** Container roda witho user nextjs (non-root), multi-stage build
- **Standalone Output:** Next.js standalone minimiza superfície de ataque
- **SSL:** Suporte a sslmode=verify-full para PostgreSQL

## 7. Plugin Security

- **isolated-vm:** Plugins executam em VM isolada with limite de memória e timeout
- **Permission Gate:** Access a data e hooks requer permissão aprovada
...
- **Permission Validtion:** ThemeDataService valida permissão antes de cada acesso
